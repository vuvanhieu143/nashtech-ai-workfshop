...
Stages are sequential. The Ralph loop may repeat implementation and verification, but it may not bypass an approval gate or change approved scope without returning to the affected stage.

## 2. Stage Map

| Stage           | Persona                                               | Inputs                                        | Output                               | Gate                   |
| --------------- | ----------------------------------------------------- | --------------------------------------------- | ------------------------------------ | ---------------------- |
| Requirements    | `agents/ba.agent.md`                                  | Product brief + template                      | `docs/requirements/US-XX.vN.md`      | BA + Human             |
| Design          | `agents/architect.agent.md`                           | Approved requirements + symbol map + template | `docs/design/US-XX.vN.md`            | Architect + Human      |
| Planning        | `agents/architect.agent.md` + tester perspective      | Approved requirements + design + template     | `docs/planning/US-XX.vN.md`          | Architect + Human      |
| Implementation  | `agents/developer.agent.md`                           | Approved planning + coding standards          | Source + tests                       | Automated verification |
| Verify / Review | `agents/tester.agent.md` + `agents/reviewer.agent.md` | Implementation + approved artifacts           | Test/review findings                 | Reviewer               |
| Human Gate      | Human                                                 | Approved artifacts + verification evidence    | Approve / Revise / Reject / Re-scope | Human                  |
...
4. **Human gate** — a human explicitly approves before entering the next stage.
...
| Test/environment infrastructure failure | Verification |
